...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mainForm = document.getElementById('form__section-1');

  // Логіка обробки форми
  mainForm.addEventListener('submit', (event) => {
    event.preventDefault(); // Зупиняємо стандартну поведінку форми

    let pSynthesis = Number(mainForm.pSynthesis.value);
    let l = Number(mainForm.l.value);
    let w = Number(mainForm.w.value);
    let pInertsPercent = Number(mainForm.pInertsPercent.value);

    // console.log("pSynthesis: ", pSynthesis, l, w, pInertsPercent);
      
    ureaConversionCalc(pSynthesis, l, w, pInertsPercent);
    // ureaConversionCalc();
  });
  // *************************************************
  const parentBox = document.querySelector('.page__section-text');
  parentBox.classList.add("hidden");
  const el1 = document.querySelector('.output-text');
  const el2 = document.querySelector('.output-text2');


  // розрахунки

  function ureaConversionCalc(pSynthesis, l, w, pInertsPercent) {
    ...
    // P synthesis, bar
    // let pSynthesis = 140;
    // let pSynthesis = mainForm.pSynthesis.value;

    // L liquid (NH3:CO2)
    // let l = 2.85;
    // let l = mainForm.l.value;

    // W liquid (H2O:CO2)
    // let w = 0.47;
    // let w = mainForm.w.value;

    // P INERTS (0%..8%) (PIN%)
    // let pInertsPercent = 8;
    // let pInertsPercent = mainForm.pInertsPercent.value;
    ...
    const liquidTableBody = document.querySelector(".liquid-table tbody");
